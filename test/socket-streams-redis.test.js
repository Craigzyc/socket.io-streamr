...
describe('Socket Streams', () => {
  ...
  beforeAll(async () => {
    ...
    // Setup Redis clients
    pubClient = new Redis(`redis://127.0.0.1:6379`);
    subClient = pubClient.duplicate();

    // Setup Redis adapter for main server
    mainIo.adapter(createAdapter(pubClient, subClient));

    // Create separate Redis-connected server
    redisServer = new Server();
    pubDuplicate = pubClient.duplicate();
    subDuplicate = subClient.duplicate();
    redisServer.adapter(createAdapter(pubDuplicate, subDuplicate));
